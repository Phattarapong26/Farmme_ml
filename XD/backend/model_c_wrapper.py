# -*- coding: utf-8 -*-
"""
Model C Wrapper v8.0 - Improved Version
Supports both v7 (old) and v8 (new improved) models
"""
...
class ModelCWrapper:
    """Wrapper for Model C - Price Prediction (v8.0 with fallback to v7.0)"""
    
    def __init__(self, prefer_v8: bool = True):
        """
        Initialize Model C Wrapper
        
        Args:
            prefer_v8: If True, try to load v8 models first, fallback to v7
        """
        self.model = None
        self.forecast_service = None
        self.model_loaded = False
        self.model_path = None
        self.prefer_v8 = prefer_v8
        
        # Try to load Model C
        self._load_model()
    
    def _load_model(self):
        """Load Model C (try v8 first, fallback to v7)"""
        if self.prefer_v8:
            success = self._load_v8_models()
            if not success:
                logger.warning("⚠️ v8 models not found, trying v7...")
                self._load_v7_models()
        else:
            self._load_v7_models()
    
    def _load_v8_models(self) -> bool:
        """Load v8.0 improved models"""
        try:
            import json
            
            models_dir = Path(__file__).parent / "models"
            
            # Check if v8 models exist
            model_low_path = models_dir / "model_c_v8_stratified_low.pkl"
            model_medium_path = models_dir / "model_c_v8_stratified_medium.pkl"
            model_high_path = models_dir / "model_c_v8_stratified_high.pkl"
            fallback_path = models_dir / "model_c_v8_fallback.pkl"
            thresholds_path = models_dir / "model_c_v8_thresholds.json"
            features_path = models_dir / "model_c_v8_features.json"
            metadata_path = models_dir / "model_c_v8_metadata.json"
            ...
            if fallback_path.exists():
                with open(fallback_path, 'rb') as f:
                    self.model_fallback = pickle.load(f)
                logger.info("✅ Loaded fallback model")
            else:
                self.model_fallback = None
                logger.warning("⚠️ No fallback model found")
            ...
            if metadata_path.exists():
                with open(metadata_path, 'r', encoding='utf-8') as f:
                    ...
                    self.model_version = "8.0.0"
                    self.algorithm = metadata.get('algorithm', 'HistGradientBoostingRegressor')
                    self.training_date = metadata.get('trained_at', 'unknown')
                    self.model_metrics = metadata.get('overall_performance', {})
            else:
                self.model_version = "8.0.0"
                self.algorithm = "HistGradientBoostingRegressor"
            ...
            logger.info(f"✅ Model C v8.0 (Improved) loaded successfully")
            logger.info(f"   Version: {self.model_version}")
            logger.info(f"   Algorithm: {self.algorithm}")
            logger.info(f"   Models: LOW, MEDIUM, HIGH + FALLBACK")
            ...
            logger.info(f"   MAE: {self.model_metrics.get('test_mae', 'N/A'):.2f} baht/kg")
            logger.info(f"   R²: {self.model_metrics.get('test_r2', 'N/A'):.4f}")
            logger.info(f"   Training date: {self.training_date}")
            
            return True
                
        except Exception as e:
            logger.error(f"Error loading v8 models: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return False
    
    def _load_v7_models(self):
        """Load v7.0 models (fallback)"""
        try:
            import json
            ...
            if not all([model_low_path.exists(), model_medium_path.exists(), model_high_path.exists()]):
                logger.error(f"❌ v7 models also not found")
                self.model_loaded = False
                return
            ...
            self.model_fallback = None  # v7 doesn't have fallback
            ...
            logger.info(f"✅ Model C v7.0 (Stratified) loaded successfully")
            logger.info(f"   Version: {self.model_version}")
            ...
            logger.error(f"Error loading v7 models: {e}")
            import traceback
            logger.error(traceback.format_exc())
            self.model_loaded = False
            self.model = None
    ...
    def predict_price(
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Using Model C {self.model_version} ({self.algorithm}) for prediction")
                ...
                model_result = self._predict_with_stratified_model(
                ...
                # Use historical data from model
                ...
                return {
                    ...
                    "model_used": f"model_c_v{self.model_version}_{self.algorithm}",
    ...
    def _predict_with_stratified_model(
        self,
        crop_type: str,
        province: str,
        days_ahead: int,
        ...
        """Make predictions using stratified models with fallback"""
        try:
            import pandas as pd
            from datetime import datetime, timedelta
            ...
            # Create features
            features = self._create_features(prices_chrono, current_price)
            ...
            for day in range(1, days_ahead + 1):
                ...
                try:
                    ...
                    # Use fallback model if category model fails
                    if self.model_fallback:
                        logger.warning(f"Category model failed, using fallback: {e}")
                        pred_price = self.model_fallback.predict(X)[0]
                        model_used = "fallback"
                    else:
                        logger.error(f"No fallback model available: {e}")
                        raise
            ...
            logger.error(f"Error in stratified model prediction: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return None
    
    def _create_features(self, prices_chrono: List[float], current_price: float) -> Dict:
        """Create features from price history"""
        features = {}
        
        # Price lags
        if len(prices_chrono) >= 30:
            features['price_lag_7'] = prices_chrono[-7]
            features['price_lag_14'] = prices_chrono[-14]
            features['price_lag_21'] = prices_chrono[-21]
            features['price_lag_30'] = prices_chrono[-30]
        else:
            features['price_lag_7'] = prices_chrono[-min(7, len(prices_chrono))]
            features['price_lag_14'] = prices_chrono[-min(14, len(prices_chrono))]
            features['price_lag_21'] = prices_chrono[-min(21, len(prices_chrono))]
            features['price_lag_30'] = prices_chrono[-min(30, len(prices_chrono))]
        
        # Moving averages (shifted by 7 days)
        if len(prices_chrono) >= 14:
            features['price_ma_7'] = np.mean(prices_chrono[-14:-7])
            features['price_std_7'] = np.std(prices_chrono[-14:-7])
            features['price_median_7'] = np.median(prices_chrono[-14:-7])
        else:
            features['price_ma_7'] = np.mean(prices_chrono)
            features['price_std_7'] = np.std(prices_chrono)
            features['price_median_7'] = np.median(prices_chrono)
        
        if len(prices_chrono) >= 21:
            features['price_ma_14'] = np.mean(prices_chrono[-21:-7])
            features['price_std_14'] = np.std(prices_chrono[-21:-7])
            features['price_median_14'] = np.median(prices_chrono[-21:-7])
        else:
            features['price_ma_14'] = features['price_ma_7']
            features['price_std_14'] = features['price_std_7']
            features['price_median_14'] = features['price_median_7']
        
        if len(prices_chrono) >= 37:
            features['price_ma_30'] = np.mean(prices_chrono[-37:-7])
            features['price_std_30'] = np.std(prices_chrono[-37:-7])
            features['price_median_30'] = np.median(prices_chrono[-37:-7])
        else:
            features['price_ma_30'] = features['price_ma_14']
            features['price_std_30'] = features['price_std_14']
            features['price_median_30'] = features['price_median_14']
        ...
        # Volatility
        if features['price_ma_7'] > 0:
            features['price_volatility_7d'] = features['price_std_7'] / features['price_ma_7']
        else:
            features['price_volatility_7d'] = 0
        
        if features['price_ma_30'] > 0:
            features['price_volatility_30d'] = features['price_std_30'] / features['price_ma_30']
        else:
            features['price_volatility_30d'] = 0
        
        # Time features (seasonal)
        now = datetime.now()
        features['dayofyear'] = now.timetuple().tm_yday
        features['month'] = now.month
        features['weekday'] = now.weekday()
        features['quarter'] = (now.month - 1) // 3 + 1
        features['week_of_month'] = (now.day - 1) // 7 + 1
        ...
        for feat in self.feature_names:
            if feat not in features:
                if 'lag' in feat or 'ma' in feat or 'std' in feat or 'median' in feat:
    ...
    def _calculate_price_range(self, predicted_price: float, confidence: float) -> tuple:
        ...
        predicted_price = max(5.0, min(500.0, predicted_price))
        range_factor = 1 - confidence
        range_width = predicted_price * range_factor * 0.3
        min_price = max(5.0, predicted_price - range_width)
        max_price = max(min_price + 1.0, predicted_price + range_width)
        return (min_price, max_price)
    
    def _clip_price(self, price: float) -> float:
        """Clip price to reasonable range"""
        if price < 5.0:
            logger.warning(f"⚠️ Low price detected: {price}, clipping to 5.0")
            return 5.0
        elif price > 500.0:
            logger.warning(f"⚠️ High price detected: {price}, clipping to 500.0")
            return 500.0
        return price
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        info = {
            "model_name": f"Model C {self.model_version} - Price Prediction",
            ...
            "has_fallback": self.model_fallback is not None if hasattr(self, 'model_fallback') else False,
            "status": "active" if self.model_loaded else "not_loaded"
        }
